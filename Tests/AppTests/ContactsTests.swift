...
final class ContactsTests: AppTestCase {
    ...
    func test_01_getUserContacts() async throws {
        ...
        try await asyncTest(.GET, "api/users/me/contacts", headers: .none, afterResponse: { res in
    ...
    func test_02_addUserContact() async throws {
        ...
        try await asyncTest(.POST, "api/users/me/contacts", beforeRequest: { req in
    ...
    func test_03_deleteUserContact() async throws {
        ...
        try await asyncTest(.DELETE, "api/users/me/contacts/\(contact.id!)", afterResponse: { res in
            XCTAssertEqual(res.status, .ok, res.body.string)
        })
        try await asyncTest(.GET, "api/users/me/contacts", headers: .none, afterResponse: { res in
            XCTAssertEqual(res.status, .ok, res.body.string)
